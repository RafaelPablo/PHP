...
function register_user_secretary($name, $username, $password)
{
  $name = $_POST['name'];
  $username = $_POST['username'];
  $password = md5($_POST['password']);
  ...
  $sql = "SELECT * FROM secretary WHERE USERNAME='$username'";
  ...
  if ($username_already_exits) {
    ...
    header('Location: ../View/createAccountSecretary.php?' . $return_get);
    die();
  }

  $sql = "INSERT INTO secretary (name,username,password) values ('$name','$username','$password')";
